 ...
 * ----------------------------------------------------------------
 */
TupleTableSlot *	//SymmetrichashJoinMade
ExecHash(HashState *node)
{
    ...
    if (!TupIsNull(slot)) { //SymmetrichashJoinMade
		hashtable->totalTuples += 1;
		econtext->ecxt_innertuple = slot;
        econtext->ecxt_outertuple = slot; //SymmetrichashJoinMade
	 ...
	 */
	return slot; //SymmetrichashJoinMade
}
...
ExecHashTableCreate(Hash *node, List *hashOperators)
{
	...
	hashtable->growEnabled = false; //SymmetrichashJoinMadeProject
...
ExecScanHashBucket(HashJoinState *hjstate,
				   ExprContext *econtext)
{
	if (hjstate->hj_fetchingFromInner) { //SymmetrichashJoinMade
