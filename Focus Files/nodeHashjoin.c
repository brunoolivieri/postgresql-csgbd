...
{
	EState	   *estate;
	HashState  *outerHashNode; //SymmetrichashJoinMade
	HashState  *innerHashNode; //SymmetrichashJoinMade
	List	   *joinqual;
	List	   *otherqual;

	TupleTableSlot *inntuple;
	TupleTableSlot *outtuple; //SymmetrichashJoinMade
	ExprContext *econtext;
	ExprDoneCond isDone;
	HashJoinTable innerHashtable; //SymmetrichashJoinMade
	HashJoinTable outerHashtable; //SymmetrichashJoinMade
	
	HeapTuple	curtuple;
	TupleTableSlot *innerTupleSlot; //SymmetrichashJoinMade
	...
	innerHashNode = (HashState *) innerPlanState(node); //SymmetrichashJoinMade
	outerHashNode = (HashState *) outerPlanState(node); //SymmetrichashJoinMade

	/*
	 * get information from HashJoin state
	 */
	innerHashtable = node->hj_InnerHashTable; //SymmetrichashJoinMade
	outerHashtable = node->hj_OuterHashTable; //SymmetrichashJoinMade
	...
	*/
	 ...
	 */
	if (innerHashtable == NULL && outerHashtable == NULL) //SymmetrichashJoinMade
	{
		/*
		 ...
		 */
		/* SymmetrichashJoinMade COMMENTED OUT ORIGINAL CODE
		...
		*/
        ...
        outerHashtable = ExecHashTableCreate((Hash *) outerHashNode->ps.plan, node->hj_HashOperators); //SymmetrichashJoinMade
        node->hj_OuterHashTable = outerHashtable; //SymmetrichashJoinMade


		/*
		 * execute the Hash node, to build the hash table
		 */
		innerHashNode->hashtable = innerHashtable; //SymmetrichashJoinMade
		outerHashNode->hashtable = outerHashtable; //SymmetrichashJoinMade
	...
	/*
	 * SymmetrichashJoinMade run the hash join process
	 */
	for (;;) {
...
ExecInitHashJoin(HashJoin *node, EState *estate)
{
    ...
    Hash       *outerHashNode; //SymmetrichashJoinMade
    Hash       *innerHashNode; //SymmetrichashJoinMade
	List	   *lclauses;
	List	   *rclauses;
	List	   *hoperators;
	ListCell   *l;
	...
	*/	
	 ...
	 */
    outerHashNode = (Hash *) outerPlan(node); //SymmetrichashJoinMade
    innerHashNode = (Hash *) innerPlan(node); //SymmetrichashJoinMade

    outerPlanState(hjstate) = ExecInitNode((Plan *) outerHashNode, estate); //SymmetrichashJoinMade
    innerPlanState(hjstate) = ExecInitNode((Plan *) innerHashNode, estate); //SymmetrichashJoinMade
...
#define HASHJOIN_NSLOTS 3
	...
	hjstate->hj_InnerTupleSlot = ExecInitExtraTupleSlot(estate); //SymmetrichashJoinMade	
...

    {
		...
		hashstate = (HashState *) outerPlanState(hjstate); //SymmetrichashJoinMade 
		slot = hashstate->ps.ps_ResultTupleSlot; //SymmetrichashJoinMade
		hjstate->hj_OuterHashTupleSlot = slot; //SymmetrichashJoinMade
    }
	...
	hjstate->hj_OuterHashTable = NULL; //SymmetrichashJoinMade
	hjstate->hj_FirstInnerTupleSlot = NULL; //SymmetrichashJoinMade

	hjstate->hj_InnerCurHashValue = 0;
	hjstate->hj_InnerCurBucketNo = 0;
	hjstate->hj_InnerCurTuple = NULL;

    hjstate->hj_OuterCurHashValue = 0; //SymmetrichashJoinMade
    hjstate->hj_OuterCurBucketNo = 0; //SymmetrichashJoinMade
    hjstate->hj_OuterCurTuple = NULL; //SymmetrichashJoinMade
    ...
    ((HashState *) outerPlanState(hjstate))->hashkeys = lclauses; //SymmetrichashJoinMade

	hjstate->js.ps.ps_OuterTupleSlot = NULL;
	hjstate->js.ps.ps_InnerTupleSlot = NULL; //SymmetrichashJoinMade
	hjstate->js.ps.ps_TupFromTlist = false;
	hjstate->hj_NeedNewOuter = true;
	hjstate->hj_NeedNewInner = true; //SymmetrichashJoinMade
	...
	hjstate->hj_innerExhausted = false; //SymmetrichashJoinMade
	hjstate->hj_outerExhausted = false; //SymmetrichashJoinMade
	hjstate->hj_foundByProbingInner = 0; //SymmetrichashJoinMade
	hjstate->hj_foundByProbingOuter = 0; //SymmetrichashJoinMade
	hjstate->hj_fetchingFromInner = true; //SymmetrichashJoinMade
...
ExecEndHashJoin(HashJoinState *node)
{
    ...
    if (node->hj_OuterHashTable) //SymmetrichashJoinMade
    {
        ExecHashTableDestroy(node->hj_OuterHashTable); //SymmetrichashJoinMade
        node->hj_OuterHashTable = NULL; //SymmetrichashJoinMade
    }
	...
	//ExecClearTuple(node->js.ps.ps_InnerTupleSlot); //SymmetrichashJoinMade
	//ExecClearTuple(node->js.ps.ps_OuterTupleSlot); //SymmetrichashJoinMade
	ExecClearTuple(node->hj_OuterTupleSlot);
	ExecClearTuple(node->hj_InnerTupleSlot); //SymmetrichashJoinMade
	ExecClearTuple(node->hj_InnerHashTupleSlot);
	ExecClearTuple(node->hj_OuterHashTupleSlot); //SymmetrichashJoinMade
	...
	{							/* if it is the first pass */
		...
		if (!TupIsNull(slot))
		{
			...
			*hashvalue = ExecHashGetHashValue(hashtable, econtext,
			...
			/*hjstate->hj_OuterNotEmpty = true; SymmetrichashJoinMade*/
...
{
	/*SymmetrichashJoinMade commented out the function inners, but left definition to watch for dependencies
	HashJoinTable hashtable = hjstate->hj_InnerHashTable;
	int			nbatch;
	int			curbatch;
	BufFile    *innerFile;
